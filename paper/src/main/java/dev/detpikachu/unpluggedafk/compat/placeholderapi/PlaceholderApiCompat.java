 ...
 *
 * <p>{@link PlaceholderApiExpansion#persist()} has to return {@code true}. An expansion registered from inside a
 * plugin is unregistered for good on a PlaceholderAPI reload otherwise, because nothing re-registers it until the
 * server restarts.
 *
 * <p>The two failure returns mean different things. {@code null} means the placeholder is not ours, and
 * PlaceholderAPI puts the raw text back so a typo stays visible. An empty string means the placeholder is ours but
 * has no value, which covers a player who is not unplugged and the null {@code OfflinePlayer} PlaceholderAPI passes
 * when it resolves text belonging to no player, such as a Discord channel topic.
 */
@ApiStatus.Internal
