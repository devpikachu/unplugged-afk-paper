 ...
 * player's UUID. That UUID therefore keys every method here, so asking whether a Bukkit {@code Player} is a bot
 * means calling {@link #isUnplugged(UUID)} with {@code player.getUniqueId()}. A player and their own bot are never
 * online at the same time, so a UUID identifies at most one of the two.
 *
 * <p>Every method is safe to call from any thread. The backing collections are concurrent, so a call from an async
 * task will not throw or observe a partially updated map. Results are point-in-time snapshots and may be stale as soon
 * as they return.
 *
...
public interface UnpluggedAfkApi {
     ...
     * normally a couple of ticks and lasts at most 100. It is disjoint from {@link #isUnplugged(UUID)}: no UUID
     * is reported by both at once, and a UUID in this state has no {@link UnpluggedPlayerInfo} yet.
     *
     ...
     *
     * <p>This is false during the short window between the player being kicked and their bot being placed. Callers
     * that mean "leave this account alone" rather than "a bot exists" should check {@link #isUnplugging(UUID)} too.
     *
     * @param uuid the player's UUID
